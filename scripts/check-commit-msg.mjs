 ...
 *
 * CLAUDE.md forbids `Co-authored-by` trailers naming an AI tool, and the
 * agents that read the file obey it. This binds the ones that do not.
 *
 * The sibling Bilikha repository learned this the expensive way: the rule was
 * written down, trailers arrived anyway, and enough of them reached GitHub to
 * attach a bot account to that repository's contributor list permanently —
 * GitHub does not retract a contribution when the commit is force-pushed out
 * of history. Ani starts with the check in place instead.
 *
 * Two modes:
 *
 ...
 *
 * Exits 1 if any message carries attribution. See ADR 0014.
 */
...
"Commit and pull request attribution", and ADR 0014.
