...
The UI design system lives next to the code it governs:
...
- **Getting started** rots fastest. Anyone who follows it and hits a snag
  should fix it in the same pull request.
- **Reference** rots whenever code changes. Prefer generated reference over
  hand-written where the option exists.
- **Guides** rot slowly. They describe a workflow, not an implementation.
- **Explanation** and **decisions** barely rot. A decision that stops being
  true is marked superseded, never deleted — the reasoning stays valuable even
